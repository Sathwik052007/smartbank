...
function handleLogin() {
    ...
    if (matchingUser) {
        ...
        localStorage.setItem("smartbank_users", JSON.stringify(users));
        sessionStorage.setItem("active_user_session", currentUser.username);
...
function triggerRequestMoney() {
    const reqUser = prompt("Enter username or mobile number to request money from:");
    if (!reqUser) return;
    ...
    // Simulate Request
    alert(`Requested ₹${amount} from node: ${reqUser}. Simulation invoice sent.`);
}
